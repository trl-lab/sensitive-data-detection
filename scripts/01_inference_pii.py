...
from utilities.detect_reflect import detect_pii, reflect_pii
...
def setup_logging(output_dir: str) -> str:
    ...
    log_filename = os.path.join(logs_dir, f"gittables_inference_log_{timestamp}.log")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Detect and reflect PII in CSV files')
    parser.add_argument(
        '--input', '-i',
        type=str,
        default=f"{PROJECT_ROOT}/data/gittables_annotated.json",
        help='Path to the folder containing CSV files (default: gittables folder)'
    )
    parser.add_argument(
        '--model', '-m',
        type=str,
    ...
    parser.add_argument(
        '--output', '-o',
        type=str,
        default=f"{PROJECT_ROOT}/results/gittables.json",
        help='Path to save the results JSON file (default: results/gittables.json)'
    )
    ...
    logging.info("GitTables Inference Configuration:")
    ...
    for key, value in tqdm(csv_data.items(), desc="Processing tables"):
        ...
        # value = detect_pii(value, generator, k=10)
        value = reflect_pii(value, generator, detect_key="pii_detection_/home/ltelkamp/detecting-sensitive-data/results/unsloth/Qwen3-8B-ft", reflect_key="pii_detection_qwen3_8B_ft_reflection_gpt_4o_mini")
        csv_data[key] = value
    ...
    logging.info("="*50)
    logging.info(f"Total tables processed: {len(csv_data)}")
    ...
    logging.info("="*50)
    logging.info(f"Model: {args.model}")
    logging.info(f"Tables processed: {len(csv_data)}")
    logging.info(f"Average time per table: {average_time_per_table:.2f} seconds")
